...
interface ErrorFormularioTurismo {
  nombre: string;
  correo: string;
  telefono: string;
  asunto: string;
...
export class Turismo {
  ...
  errorFormulario = '';
  isSubmitting = false;
  fieldErrors: ErrorFormularioTurismo = {
    nombre: '',
    correo: '',
    telefono: '',
    asunto: '',
  ...
  private validarCampo(formulario: NgForm, nombreCampo: keyof ErrorFormularioTurismo): string {
    ...
    switch (nombreCampo) {
      ...
      case 'asunto':
        ...
        return '';
      case 'mensaje':
        if (!valor) return 'El detalle de la consulta es obligatorio.';
        if (valor.length < 20) return 'El detalle debe tener al menos 20 caracteres.';
        return '';
      default:
        return '';
    }
  ...
  private validarFormulario(formulario: NgForm): boolean {
    ...
    const campos: (keyof ErrorFormularioTurismo)[] = ['nombre', 'correo', 'telefono', 'asunto', 'mensaje'];
